<x-layouts.knowledge :title="'Edit: '.$article->title" currentView="create">
    @php
        $org = auth()->user()->currentOrganization();
        $kbAttach = auth()->user()->is_super_admin
            || ($org && app(\App\Services\PlanService::class)->canUse($org, 'knowledge_base', 'attachments'));
    @endphp
    <h1 class="text-[22px] font-bold text-white/90 mb-2">Edit article</h1>
    <p class="text-[12px] text-white/35 mb-6">{{ $article->title }}</p>
    <form method="post" action="{{ route('knowledge.articles.update', $article) }}" class="space-y-5 max-w-4xl" onsubmit="if (typeof tinymce !== 'undefined') tinymce.triggerSave();">
        @csrf
        @method('PUT')
        <div>
            ...
            <select name="knowledge_category_id" required class="w-full max-w-md rounded-lg bg-white/[0.06] border border-white/[0.1] px-3 py-2 text-[14px] text-white/85">
                @foreach($categories as $cat)
                    <option value="{{ $cat->id }}" @selected((int) old('knowledge_category_id', $article->knowledge_category_id) === $cat->id)>{{ $cat->name }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="block text-[12px] font-medium text-white/45 mb-1.5">Title</label>
            <input name="title" value="{{ old('title', $article->title) }}" required class="w-full rounded-lg bg-white/[0.06] border border-white/[0.1] px-3 py-2 text-[14px] text-white/85"/>
        </div>
        <div>
            <label class="block text-[12px] font-medium text-white/45 mb-1.5">Slug</label>
            <input name="slug" value="{{ old('slug', $article->slug) }}" class="w-full rounded-lg bg-white/[0.06] border border-white/[0.1] px-3 py-2 text-[14px] text-white/85"/>
        </div>
        <div>
            <label class="block text-[12px] font-medium text-white/45 mb-1.5">Tags</label>
            <input name="tag_input" value="{{ old('tag_input', $article->tags->pluck('name')->join(', ')) }}" class="w-full rounded-lg bg-white/[0.06] border border-white/[0.1] px-3 py-2 text-[14px] text-white/85"/>
        </div>
        <div>
            <label class="block text-[12px] font-medium text-white/45 mb-1.5">Excerpt</label>
            <textarea name="excerpt" rows="2" class="w-full rounded-lg bg-white/[0.06] border border-white/[0.1] px-3 py-2 text-[14px] text-white/85">{{ old('excerpt', $article->excerpt) }}</textarea>
        </div>
        <div>
            <label class="block text-[12px] font-medium text-white/45 mb-1.5">Content</label>
            <textarea id="kb-body-html" name="body_html" rows="16" class="w-full rounded-lg bg-white/[0.06] border border-white/[0.1] text-[14px] text-white/85">{{ old('body_html', $article->body_html) }}</textarea>
        </div>
        <div class="flex flex-wrap gap-4 items-center">
            ...
            <label class="flex items-center gap-2 text-[13px] text-white/55">
                <input type="checkbox" name="pinned" value="1" @checked(old('pinned', $article->pinned)) class="rounded border-white/20 bg-white/5"/>
                Pin in category
            </label>
            @endif
        </div>
        @if($kbAttach && $article->attachments->isNotEmpty())
            <div>
                <p class="text-[12px] font-medium text-white/45 mb-2">Existing attachments</p>
                <ul class="text-[12px] text-sky-300/90 space-y-1">
                    @foreach($article->attachments as $att)
                        <li><a href="{{ route('knowledge.files.download', $att) }}" target="_blank">{{ $att->original_name }}</a></li>
                    @endforeach
                </ul>
            </div>
        @endif
        ...
        <div x-data="kbAttachments()" class="space-y-2">
            <label class="block text-[12px] font-medium text-white/45">Add attachments</label>
            ...
            <ul class="text-[12px] text-white/50 space-y-1">
                <template x-for="id in ids" :key="id">
                    <li x-text="'New attachment #' + id"></li>
                </template>
            </ul>
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <a href="{{ route('knowledge.articles.show', $article) }}" class="rounded-lg border border-white/[0.1] text-white/50 px-5 py-2 text-[13px]">Cancel</a>
        </div>
    </form>
