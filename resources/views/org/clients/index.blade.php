<x-layouts.smartprojects currentView="clients" :title="'Clients — ' . $organization->name">
<div class="max-w-5xl mx-auto px-4 py-6" x-data="clientsManager()">

    <div class="flex items-center justify-between mb-2">
        <div>
            <p class="text-[10px] font-semibold text-white/25 uppercase tracking-widest">{{ $organization->name }}</p>
        ...
        @if(app(\App\Services\PermissionService::class)->userCan(auth()->user(), 'org.clients.manage', $organization))
    ...
    @if($clients->isEmpty())
        <div class="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p class="text-xs text-white/18 mb-4">Add prospects for pre-sales; approve and hire to create a delivery project.</p>
            @if(app(\App\Services\PermissionService::class)->userCan(auth()->user(), 'org.clients.manage', $organization))
    ...
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <a href="{{ route('org.clients.show', [$organization, $client]) }}"
               class="group bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.07] hover:border-white/[0.12] rounded-xl p-4 transition-all">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        @endif
                        <span class="inline-block mt-1.5 text-[9px] px-2 py-0.5 rounded-full bg-white/[0.06] text-white/40 font-medium">{{ $client->stageLabel() }}</span>
                    </div>
                </div>
    ...
    <div x-show="showCreate" x-cloak
        ...
        <div class="relative bg-[#16162A] border border-white/[0.12] rounded-2xl w-full max-w-md p-6 shadow-2xl"
            ...
            <div class="flex items-center justify-between mb-5">
                ...
                <button type="button" @click="showCreate = false" class="w-7 h-7 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] text-white/40 hover:text-white/70 flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <form method="POST" action="{{ route('org.clients.store', $organization) }}" class="space-y-3">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="2" placeholder="Pre-sales notes, requirements…"
