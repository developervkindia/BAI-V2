...
<head>
    ...
    <title>{{ isset($role) ? 'Edit Role' : 'Create Role' }} - {{ $organization->name }}</title>
...
<body class="h-full bg-[#0D0D18] text-white antialiased">
    ...
    {{-- Page Content --}}
    <div class="max-w-5xl mx-auto px-6 py-8" x-data="roleForm()">

        {{-- Header --}}
        <div class="mb-8">
            <h1 class="text-[22px] font-semibold text-white/85">
                {{ isset($role) ? 'Edit Role' : 'Create Role' }}
            </h1>
            <p class="text-white/40 text-[13px] mt-1">
        ...
        <form action="{{ isset($role) ? route('roles.update', [$organization, $role]) : route('roles.store', $organization) }}"
            ...
            <div class="bg-[#111120] border border-white/[0.07] rounded-2xl p-6 mb-6">
                <h2 class="text-[15px] font-semibold text-white/85 mb-5">Role Details</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text"
                               ...
                               class="w-full bg-white/[0.05] border border-white/[0.1] text-white/75 rounded-xl px-3.5 py-2.5 text-[13px] placeholder-white/20 focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/25 transition @if(isset($role) && $role->is_system) opacity-50 cursor-not-allowed @endif">
                    ...
                    {{-- Description --}}
                    <div>
                        <label for="description" class="block text-[11px] font-semibold text-white/38 uppercase tracking-wider mb-2">
                            Description
                        </label>
                        <textarea id="description"
                                  ...
                                  class="w-full bg-white/[0.05] border border-white/[0.1] text-white/75 rounded-xl px-3.5 py-2.5 text-[13px] placeholder-white/20 focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/25 transition resize-none">{{ old('description', $role->description ?? '') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-[#111120] border border-white/[0.07] rounded-2xl p-6 mb-6">
                <div class="flex items-center justify-between mb-5">
                    <div>
                        <h2 class="text-[15px] font-semibold text-white/85">Permissions</h2>
                    ...
                    <div class="flex gap-2 border-b border-white/[0.06] pb-0">
                        @foreach($permissions as $productKey => $productPerms)
                            <button type="button"
                                ...
                                :class="activeProductTab === '{{ $productKey }}' ? 'border-orange-500 text-white/85' : 'border-transparent text-white/35 hover:text-white/60'"
                                            ...
                                            @foreach($groupPerms as $permission)
                                                <label class="flex items-start gap-3 p-2.5 rounded-lg hover:bg-white/[0.03] transition cursor-pointer">
                                                    <input type="checkbox" name="permissions[]" value="{{ $permission->id }}"
                                                           ...
                                                           class="mt-0.5 w-3.5 h-3.5 rounded border-white/20 bg-white/[0.05] text-orange-500 focus:ring-orange-500/25 focus:ring-offset-0">
                                                    <div>
